...
import { ArrowLeft, Calendar, User, Tag, CheckCircle2, ChevronRight, Share2, ArrowRight } from "lucide-react";
  ...
  if (!project) {
    return (
      <div className="min-h-screen bg-[#0B0F14] flex flex-col items-center justify-center p-6 text-center">
        <h1 className="text-4xl font-bold text-[#F8FAFC] mb-4 font-outfit">Project Not Found</h1>
        <p className="text-[#94A3B8] mb-8">The project you are looking for does not exist.</p>
        <Link href="/projects" className="btn-ltr-blue px-8 py-3 rounded-xl flex items-center gap-2">
          ...
          <div className="container mx-auto px-6 max-w-7xl relative z-10">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <div className="flex flex-wrap gap-8 items-center text-sm font-bold tracking-widest uppercase text-white/60">
                <div className="flex items-center gap-2"><User size={16} className="text-[#3B82F6]" /> {project.clientName}</div>
                <div className="flex items-center gap-2"><Tag size={16} className="text-[#3B82F6]" /> {project.category}</div>
              </div>
            </motion.div>
          </div>
        </section>
                ...
                {project.galleryImageUrls && project.galleryImageUrls.length > 0 && (
                  <div className="grid grid-cols-1 gap-10">
                    {project.galleryImageUrls.map((url, i) => (
                      <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative aspect-video rounded-3xl overflow-hidden border border-white/10 shadow-2xl">
              ...
              <div className="lg:col-span-4">
                <div className="sticky top-32 glass-strong p-10 rounded-[32px] border border-[#3B82F6]/20">
                  ...
                  <div className="space-y-6 text-sm">
                    <div><p className="text-[#3B82F6] font-black uppercase tracking-widest text-[10px] mb-1">Client</p><p className="text-white font-bold">{project.clientName}</p></div>
                    <div><p className="text-[#3B82F6] font-black uppercase tracking-widest text-[10px] mb-1">Vertical</p><p className="text-white font-bold">{project.category}</p></div>
                    <div><p className="text-[#3B82F6] font-black uppercase tracking-widest text-[10px] mb-1">Date</p><p className="text-white font-bold">{new Date(project.createdAt as string).toLocaleDateString('en-US', { year: 'numeric', month: 'long' })}</p></div>
                  </div>
